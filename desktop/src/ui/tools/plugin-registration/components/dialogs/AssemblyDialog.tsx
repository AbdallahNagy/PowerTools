...
export function AssemblyDialog({ assembly, connectionName, onClose, onVerified, refreshCatalog }: AssemblyDialogProps) {
  ...
  const close = () => { setFile(null); setInspection(null); setPreflight(null); onClose(); };
  const selectFile = async (selected: File | null) => {
    setFile(selected); setInspection(null); setPreflight(null);
    if (!selected) return;
    const result = await mutations.analyze.mutateAsync(selected);
    setInspection(result);
  };
  const preview = async () => { if (file && draft) setPreflight(await mutations.preflight.mutateAsync({ file, draft })); };
  const confirm = async () => {
