...
export function MutationImpactPreviewDialog({ title, environment, component, changes, warnings, blockers, confirmation, executing, onCancel, onConfirm }: {
  ...
  executing: boolean; onCancel: () => void; onConfirm: () => void;
}) {
  return <Modal open title={title} onClose={onCancel} widthClass="max-w-lg"><div role="dialog" aria-label={title} className="flex flex-col gap-3">
    ...
    <div className="flex justify-end gap-2"><Button variant="secondary" onClick={onCancel}>Cancel</Button><Button disabled={blockers.length > 0 || executing} onClick={onConfirm}>Confirm</Button></div>
  </div></Modal>;
}
