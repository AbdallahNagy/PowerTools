...
namespace Data8.PowerPlatform.Dataverse.Client
{
    ...
    public class OnPremiseClient : IOrganizationServiceAsync2
    {
        ...
        public OnPremiseClient(string url, ClientCredentials credentials)
        {
            ...
            // Use the address published by Dynamics for the organization service. The
            // path is part of the ADFS relying-party identifier and can be case-sensitive.
            _url = ResolveOrganizationServiceUrl(url, wsdl);
        ...
        private static string ResolveOrganizationServiceUrl(string requestedUrl, IEnumerable<Definitions> wsdls)
        {
            ...
            return wsdls
                ...
                .Where(service => service.Ports != null)
                .SelectMany(service => service.Ports)
                .Select(port => port.Address?.Location)
                .Where(location => Uri.TryCreate(location, UriKind.Absolute, out _))
                .Select(location => new Uri(location))
                .FirstOrDefault(candidate =>
                    candidate.Scheme.Equals(requestedUri.Scheme, StringComparison.OrdinalIgnoreCase) &&
                    candidate.Authority.Equals(requestedUri.Authority, StringComparison.OrdinalIgnoreCase) &&
                    candidate.AbsolutePath.Equals(requestedUri.AbsolutePath, StringComparison.OrdinalIgnoreCase))
                ?.AbsoluteUri ?? requestedUrl;
        }
        ...
        private ClaimsBasedAuthClient ConnectFederated(string url, ClientCredentials credentials, List<Policy> policies)
        {
            ...
            var usernameWsTrust13PolicyIds = issuerPolicies
                .Where(p =>
                    p.FindPolicyItem<SignedEncryptedSupportingTokens>()?.Policy.FindPolicyItem<UsernameToken>() != null &&
                    p.FindPolicyItem<Trust13>() != null)
                .Select(p => p.Id)
                .Where(id => !String.IsNullOrWhiteSpace(id))
                .ToHashSet(StringComparer.Ordinal);

            if (usernameWsTrust13PolicyIds.Count == 0)
            {
            ...
            var usernameWsTrust13Bindings = issuerBindings
                .Where(b =>
                    b.PolicyReference?.Uri != null &&
                    usernameWsTrust13PolicyIds.Contains(b.PolicyReference.Uri.TrimStart('#')))
                .Select(b => b.Name)
                .Where(name => !String.IsNullOrWhiteSpace(name))
                .ToHashSet(StringComparer.Ordinal);

            if (usernameWsTrust13Bindings.Count == 0)
            {
            ...
            var usernameWsTrust13Ports = issuerPorts
                .Where(p =>
                    p.Address?.Location != null &&
                    p.Binding != null &&
                    usernameWsTrust13Bindings.Contains(p.Binding.Split(':').Last()))
                .Select(p => p.Address.Location)
                .Where(location => Uri.TryCreate(location, UriKind.Absolute, out _))
                .ToList();

            var usernameWsTrust13Port = usernameWsTrust13Ports
                .Select(location => new Uri(location))
                .Where(endpoint => endpoint.Scheme.Equals(Uri.UriSchemeHttps, StringComparison.OrdinalIgnoreCase))
                .OrderByDescending(endpoint =>
                    endpoint.AbsolutePath.EndsWith("/usernamemixed", StringComparison.OrdinalIgnoreCase))
                .FirstOrDefault();

            if (usernameWsTrust13Port == null)
            {
                var found = usernameWsTrust13Ports.Count == 0
                    ? "none"
                    : String.Join(", ", usernameWsTrust13Ports);
                throw new InvalidOperationException(
                    "Unable to find an HTTPS WS-Trust 1.3 username token port. Found: " + found);
            }

            // Create the SOAP client to authenticate against the STS
            var client = new ClaimsBasedAuthClient(url, usernameWsTrust13Port.AbsoluteUri);
